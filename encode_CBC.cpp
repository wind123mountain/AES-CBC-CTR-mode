#include <bits/stdc++.h>
#include "encode.h"

using namespace std;

#define N_block 5000000
unsigned char input[N_block<<4];
unsigned char output[N_block<<4];
unsigned char IV[16] = {255,255,255,255,255,255,255,255,255,255,255,255,255,255,255,255};
unsigned char key[16] = {255,255,255,255,255,255,255,255,255,255,255,255,255,255,255,255};
unsigned char expandedKey[176];

void creatIV_expandedKey(){
	//lay key
	string str;
	ifstream infile;
	infile.open("keyfile", ios::in | ios::binary);
	if (infile.is_open()){
		getline(infile, str);
		infile.close();
	}
	else cout << "Open keyfile error!";

	istringstream hex_chars_stream(str);
	int i = 0;
	unsigned int c;
	while (hex_chars_stream >> hex >> c){
		key[i] = c;
		i++;
	}
	
	//expandedKey
	KeyExpansion(key, expandedKey);

	// sinh IV ngau nhien tu 0 -> 255
    srand(time(NULL));
    for(int i = 0; i != 16; i++){
        IV[i] = rand() % 256;
    }
    
}

int main(int argc, char **argv){
	clock_t s = clock();
	
    unsigned long long dung_luong = 0;
    fstream fin, fout;
	char *input_file = "e_inp.txt", *output_file = "cipher.txt";
	if(argc > 1){
		input_file = argv[1];
	}
	// tinh dung luong
    FILE *fp;
    fp = fopen(input_file, "rb");
    fseek(fp, 0, SEEK_END);
    dung_luong = ftell(fp);
    fclose(fp);
    fin.open(input_file, ios::in | ios::binary);
    fout.open(output_file, ios::out | ios::binary);
    
    //sinh IV, mo rong khoa
    creatIV_expandedKey();
    // ghi IV ra file
    fout.write((char*)IV, 16);
    int n = dung_luong;
    unsigned char *inIV = IV;
    unsigned char inASE[16];
    bool endFile = false;
    while(!fin.eof()){
    	fin.read((char*)input, sizeof(input));
    	int len = 0;
        if(n > N_block<<4){
            len = N_block;
            n = n - N_block<<4;
        }
        else{
            len = n/16;
            endFile = true;
        }
        
        for(int i = 0; i != len; i++){
        	unsigned char *temp = input + (i<<4);
        	for(int j = 0; j != 16; j++){
        		inASE[j] = temp[j] ^ inIV[j];
			}
        	AESEncrypt(inASE, expandedKey, output + (i<<4));
        	inIV = output + (i<<4);
		}
		
		fout.write((char*)output, len<<4);
		
		//padding
		if(endFile){
			int n_mod = n%16;
			if(n_mod == 0){
				unsigned char pad[16];
				unsigned char out_pad[16];
				memset(pad, 16, sizeof(pad));
				for(int i = 0; i != 16; i++){
					inASE[i] = pad[i] ^ inIV[i];
				}
				AESEncrypt(inASE, expandedKey, out_pad);
				fout.write((char*)out_pad, 16);
			}
			else{
				int n_pad = 16 - n_mod;
				for(int i = n_mod; i != 16; i++){
					input[(len<<4) + i] = n_pad;
				}
				unsigned char *temp = input + (len<<4);
	        	for(int j = 0; j != 16; j++){
	        		inASE[j] = temp[j] ^ inIV[j];
				}
				AESEncrypt(inASE, expandedKey, output+(len<<4));
				fout.write((char*)(output+(len<<4)), 16);
			}	
		}
	}
	
	clock_t e = clock();
	fin.close();
	fout.close();
	cout<<"\nDung luong file ma hoa: "<<dung_luong<<" bytes\n";
	cout<<"Thoi gian ma hoa: "<<(double)(e-s)/CLOCKS_PER_SEC<< " second (s)\n";
	return 0;
}
